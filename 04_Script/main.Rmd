...
```{r}
################################################################################
# main.R - HCC再発予測モデル構築メインスクリプト
# 
# Description: 
#   全パイプラインを統合実行
#   データ読み込みからモデル評価・可視化まで
#
# Author: Takuto Yoshida
# Date: 2024-11-07
################################################################################
...
# 設定ファイルから読み込み（setwd()は使わない）
config <- yaml::read_yaml("/Users/yoshidatakuto/Dropbox/Hokkaido University/大学院医学院/Borderline HCC/Prediction model_Project1/04_Script/config/settings.yaml")
PROJECT_DIR <- config$paths$base_dir

# 作業ディレクトリの確認のみ（変更はしない）
current_wd <- getwd()
...
message(sprintf("  ✓ Configuration loaded from: %s", basename(config_path)))
message(sprintf("  ✓ Variables loaded from: %s", basename(variables_path)))
...
message(sprintf("\n  ✓ Successfully built %d/%d models", 
               n_successful, length(models)))
```

```{r}
# ========================================================================
# 5.5 評価前のデータクリーニング（修正版）
# ========================================================================
...
clean_for_evaluation <- function(data) {
  ...
  vars_to_remove <- c(
    # 日付系の不要変数
    "first_visit_dt", "adm_dt",  # ← これらは本当に不要
    # BMI_category関連
  ...
  # 2. recur_dateは欠損のまま残す（再発なし情報として重要）
  # ただし、モデルには使わないので問題なし
...
if (train_na + val_na + test_na == 0) {
  message("\n✅ Perfect! No problematic NAs remaining!")
...
for (set_name in c("train", "val", "test")) {
  if (!is.null(split_data_fixed[[set_name]])) {
    ...
    message(sprintf("\n%s:", set_name))
    message(sprintf("  Dimensions: %d × %d", nrow(data), ncol(data)))
...
if (exists("evaluation_results") && !is.null(evaluation_results$summary)) {
  message("📊 Evaluation results summary:")
  ...
  if (length(best_idx) > 0) {
    ...
    message(sprintf("\n🏆 Best model: %s (C-index = %.3f)", 
                   best_model$model, best_model$c_index_test))
  }
}

# 2. 簡易版の可視化関数（C-indexベースのみ）
create_simple_visualizations <- function(evaluation_results, models, output_dir) {
  ...
  tryCatch({
    ...
    plots$comparison <- p
    message("  ✓ Model comparison plot created")
    
  }, error = function(e) {
    message(sprintf("  ✗ Error in comparison plot: %s", e$message))
  })
  ...
  tryCatch({
    ...
    if (length(var_frequency) > 0) {
      ...
      message("  ✓ Variable importance plot created")
    }
    
  }, error = function(e) {
    message(sprintf("  ✗ Error in variable plot: %s", e$message))
  })
  ...
  tryCatch({
    ...
    message("  ✓ Results table saved")
    
  }, error = function(e) {
    message(sprintf("  ✗ Error in table: %s", e$message))
  })
...
message(sprintf("\n✓ Created %d visualizations", n_plots))
```

```{r}
# ========================================================================
# 7.5 追加分析：ROCカーブ、DCA、キャリブレーション
# ========================================================================
...
message("📊 Summary of Additional Analysis:")
    ...
    for (var in common_vars) {
      if (var %in% names(data)) {
        ...
        if (is.factor(data[[var]])) {
          X_test[, var] <- as.numeric(data[[var]]) - 1  # 0/1に変換
        } else {
...
if (!is.null(roc_plot_fixed)) {
  ...
  if (exists("comprehensive_performance")) {
    ...
    message("✓ Updated performance table saved")
  }
}

message("\n✅ LASSO AUC issue fixed!")
```
...
# ベストモデルの詳細表示（既存のコードそのまま）
if (exists("evaluation_results") && !is.null(evaluation_results$summary)) {
  best_idx <- which.max(evaluation_results$summary$c_index_test)
  if (length(best_idx) > 0) {
    best <- evaluation_results$summary[best_idx, ]
    message(sprintf("\n🏆 BEST MODEL: %s", best$model))
...
message("\nGenerated Files:")
output_files <- list(
  "📊 Data" = file.path(paths$output, "processed_data.rds"),
  "🔀 Split" = file.path(paths$output_models, "data_split.rds"),
  "🤖 Models" = file.path(paths$output_models, "all_models.rds"),
  "📈 Evaluation" = file.path(paths$output_models, "evaluation_results.rds"),
  "🔬 Additional Analysis" = file.path(paths$output_models, "additional_analysis_results.rds"),
  "📋 Summary Table" = file.path(paths$output_tables, "model_performance.csv"),
  "📉 ROC Curves" = file.path(paths$output_figures, "roc_curves_all_models.pdf"),
  "📊 DCA" = file.path(paths$output_figures, "dca_comparison.pdf"),
  "🎯 Calibration" = file.path(paths$output_figures, "calibration_plot.pdf"),
  "🎨 Combined Plot" = file.path(paths$output_figures, "combined_evaluation.pdf")
)
...
message(sprintf("\n🎉 Results are available in: %s", paths$output))
message("\nNext steps:")
...
message(sprintf("\n📄 Session info saved to: %s", basename(session_file)))
message("\n================================================================================\n")
```
...
create_table1 <- function(data_list, output_file = NULL) {
  ...
  table1_print <- print(
    table1,
    nonnormal = vars_continuous,  # 中央値（IQR）で表示
    quote = FALSE,
    noSpaces = TRUE,
    printToggle = FALSE,
    showAllLevels = TRUE,
    cramVars = c("stage_diag", "ALBIgrade"),  # 一行にまとめる
    test = TRUE,  # p値を計算
    smd = TRUE    # SMDも計算
  )
  ...
  if (!is.null(output_file)) {
    ...
    message(sprintf("✓ Table 1 saved to: %s", basename(output_file)))
  }
...
create_table2 <- function(models, model_names = NULL, output_file = NULL) {
  ...
  if (length(all_coef_tables) > 0) {
    ...
    names(final_table) <- c("Model", "Variable", "β", "SE", 
                            "HR", "95% CI", "P-value", "")
    
    # CSVとして保存
    if (!is.null(output_file)) {
      write.csv(final_table, output_file, row.names = FALSE)
      message(sprintf("✓ Table 2 saved to: %s", basename(output_file)))
    }
...
if (require(openxlsx, quietly = TRUE)) {
  ...
  message("\n✓ Both tables saved to: manuscript_tables.xlsx")
}

message("\n✅ Manuscript tables created successfully!")
```
...
library(cmprsk)
# optional
safelv <- function(x) as.integer(!is.na(x) & x %in% c(TRUE, 1, "1", "Yes", "再発"))

## 1) KM/Log-rank 用の二値イベントを作る（再発=1, その他=0）
test_data <- split_data_fixed$test

# vital_status が文字なら欠損対策
if (!"vital_status" %in% names(test_data)) test_data$vital_status <- NA_character_

# KM用：再発だけをイベント扱い
test_data$km_event <- safelv(test_data$recur_2y)

# 競合リスク用：0=打切り, 1=再発, 2=再発なし死亡
test_data$event_type <- with(test_data, ifelse(
  safelv(recur_2y) == 1, 1,
  ifelse(!is.na(vital_status) & vital_status %in% c("死","死亡","Dead","deceased"), 2, 0)
))
test_data$event_type <- as.integer(test_data$event_type)

## リスク群（既存の関数でOK・NA除去も）
test_data$risk_group <- create_risk_groups(best_model, test_data)
test_data$risk_group <- droplevels(test_data$risk_group)

## 2) Kaplan–Meier と log-rank（←ここは km_event を使う）
km_fit <- survfit(Surv(time2y, km_event) ~ risk_group, data = test_data)

survdiff_result <- survdiff(Surv(time2y, km_event) ~ risk_group, data = test_data)
p_value_km <- pchisq(survdiff_result$chisq,
                     df = nlevels(test_data$risk_group) - 1,
                     lower.tail = FALSE)
...
km_plot <- ggsurvplot(
  ...
  xlab = "Time (months)", ylab = "Recurrence-free probability",
  xlim = c(0,24), break.time.by = 6, ggtheme = theme_minimal()
)
print(km_plot)

## 3) 競合リスク（cuminc & Gray）
# cmprsk::cuminc は fstatus に 0(打切り)/1..K を要求
stopifnot(all(test_data$event_type %in% c(0,1,2)))
cif <- cuminc(ftime = test_data$time2y,
              fstatus = test_data$event_type,
              group = test_data$risk_group,
              cencode = 0)

# Gray検定の取り方（全体 or ペアごとで行名が変わるため安全に）
tests <- as.data.frame(cif$Tests)
if ("pv" %in% names(tests)) {
  gray_p <- tests$pv[1]              # 先頭=全体のp（群>2の場合）
} else {
  gray_p <- tests[1, ncol(tests)]    # 互換的 fallback
}
message(sprintf("Gray's test p-value: %.4f", gray_p))

plot(cif, xlab="Time (months)", ylab="Cumulative incidence",
     xlim=c(0,24), ylim=c(0,1), lty=1, lwd=2,
     col=c("#00BFC4","#F8766D","#C77CFF"))
legend("topleft", legend=c("Low risk","Medium risk","High risk"),
       col=c("#00BFC4","#F8766D","#C77CFF"), lty=1, lwd=2)

## 4) Fine–Gray 回帰（riskRegression 版）
if (requireNamespace("riskRegression", quietly = TRUE)) {
  library(riskRegression)
  # Hist(time, status) で cause=1（=再発）を明示
  fg <- FGR(Hist(time2y, event_type) ~ risk_group,
