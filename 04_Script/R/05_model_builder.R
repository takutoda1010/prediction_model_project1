...
################################################################################
# 05_model_builder.R - モデル構築処理
# 
# Description: 
#   各種変数選択手法によるモデル構築
#   Stepwise, LASSO, RFE, 単変量スクリーニング, 事前定義モデル
#   Cox比例ハザードモデルベース
#
...
#' モデル用データの準備
#' @param df データフレーム
#' @param outcome_time 時間変数名
#' @param outcome_event イベント変数名
#' @param variables 使用する変数リスト
#' @param config 設定リスト
#' @return モデル用データ
prepare_model_data <- function(df, 
  ...
  # ===== 新規追加：欠損値処理を先に実行 =====
  # handle_missing_valuesを呼び出して欠損値を補完
  ...
  message(sprintf("  Final dataset: %d rows × %d variables", 
...
build_stepwise_model <- function(train_data,
  ...
  message(sprintf("\n🔄 Building Stepwise Cox model (%s, %s)...", 
                 direction, criterion))
...
#' LASSO Cox回帰モデル
#' @param train_data 訓練データ
#' @param alpha Elastic Netパラメータ（1=LASSO, 0=Ridge）
#' @param standardize 標準化するか
#' @param nfolds CV分割数
#' @param type.measure 評価指標
#' @return モデルオブジェクト
build_lasso_model <- function(train_data,
  ...
  message(sprintf("\n🎯 Building LASSO Cox model (alpha=%.1f)...", alpha))
  ...
  # Cross-validation
  # lambda候補の自動生成、内部cv、各lambda候補の評価、最終lambdaの決定をやってくれる
  set.seed(123)  # 再現性のため
...
# ========================================================================
# 4. LASSO with Constraint (変数数制約版)
# ========================================================================

#' 変数数制約付きLASSO
#' @param train_data 訓練データ
#' @param target_nvars 目標変数数
#' @param max_nvars 最大変数数
#' @return モデルオブジェクト
build_lasso_constrained <- function(train_data,
                                   target_nvars = 5,
                                   max_nvars = 6) {
  
  message(sprintf("\n🎯 Building constrained LASSO (target=%d vars)...", 
                 target_nvars))
...
#' 単変量スクリーニング
#' @param train_data 訓練データ
#' @param p_threshold p値の閾値
#' @param stop_p 停止p値
#' @param max_vars 最大変数数
#' @return モデルオブジェクト
build_univariate_screen_model <- function(train_data,
  ...
  message(sprintf("\n🔍 Building Univariate Screening model (p<%.2f)...", 
                 p_threshold))
  ...
  univariate_results <- data.frame(
    variable = character(),
    original_var = character(),  # ← 追加：元の変数名を保持
    coef = numeric(),
    hr = numeric(),
    se = numeric(),
    z = numeric(),
  ...
  # ← 修正：元の変数名を使用（stage_diagBR2ではなくstage_diag）
...
#' RFEによる変数選択
#' @param train_data 訓練データ
#' @param min_vars 最小変数数
#' @param step_size 一度に削除する変数数
#' @return モデルオブジェクト
build_rfe_model <- function(train_data,
                           min_vars = 5,
                           step_size = 1) {
  
  message("\n♻️ Building RFE Cox model...")
...
build_all_models <- function(split_data, 
                            config = NULL, 
                            variables = NULL,
                            variable_set_name = "postop_pathology_custom") {  # ← 追加
  
  message("\n========================================")
  message("Building All Models")
  message(sprintf("Using variable set: %s", variable_set_name))  # ← 追加
  message("========================================")
  ...
  # 訓練データの準備（修正: candidate_varsを渡す）
  train_data <- prepare_model_data(
    split_data$train,
    outcome_time = "time2y",
    outcome_event = "recur_2y",
    variables = candidate_vars,  # ← 追加: YAMLの変数を渡す
    config = config
  )
  # モデルリスト
  models <- list()
  ...
  for (model_name in active_models) {
    ...
    tryCatch({
      ...
      message(sprintf("  ❌ Error building %s: %s", model_name, e$message))
...
save_models <- function(models, file_path) {
  ...
  message(sprintf("  ✓ Models saved to: %s", file_path))
}
...
load_models <- function(file_path) {
  ...
  message(sprintf("  ✓ Models loaded from: %s", file_path))
