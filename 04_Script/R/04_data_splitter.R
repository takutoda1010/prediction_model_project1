...
################################################################################
# 04_data_splitter.R - データ分割処理
# 
# Description: 
#   訓練・検証・テストデータの分割
#   時間的分割（予測期間を考慮）、ランダム分割、層別化分割
#   クロスバリデーション用分割
#
# Author: Takuto Yoshida
# Date: 2024-11-07
# Revised: 2024-11-07 - 予測期間を考慮した時間的分割を追加
################################################################################
...
#' 予測期間を考慮した時間的データ分割
#' @param df データフレーム
#' @param date_col 分割基準となる日付列名（手術日など）
#' @param prediction_months 予測期間（月数）
#' @param reference_date 基準日（デフォルト：今日）
#' @param train_ratio 訓練データの割合
#' @param val_ratio 検証データの割合
...
temporal_split <- function(df, 
  ...
  message("\n📅 Performing temporal split with prediction window...")
  ...
  # カットオフ日の計算（予測期間分遡る）
  cutoff_date <- reference_date - (prediction_months * 30.44)  # 月を日に変換
  ...
  if (n_excluded > 0) {
    ...
    message(sprintf("  ⚠ Excluded %d cases with %s after cutoff", 
  ...
  if (n_na > 0) {
    message(sprintf("  ⚠ %d rows with missing %s were excluded", n_na, date_col))
  }
  ...
  message(sprintf("\n  ✓ Eligible cases: %d (before %s)", n_total, cutoff_date))
  message(sprintf("  ✓ Train: n=%d (%s to %s)", 
                 n_train, train_range[1], train_range[2]))
  message(sprintf("  ✓ Val:   n=%d (%s to %s)", 
                 n_val, val_range[1], val_range[2]))
  message(sprintf("  ✓ Test:  n=%d (%s to %s)", 
...
#' 予測期間の要件を確認
#' @param split_result 分割結果
#' @param date_col 日付列名
#' @param reference_date 基準日
#' @param prediction_months 予測期間
verify_prediction_window <- function(split_result, date_col, reference_date, prediction_months) {
  ...
  for (set_name in c("train", "val", "test")) {
    ...
    # 最新の手術日からの経過期間
    # set_dataの中から最も新しい日付を取得
    latest_date <- max(set_data[[date_col]], na.rm = TRUE)
    # reference_dateからそのセットの最も新しい日付を引き算し経過期間を計算
    months_elapsed <- as.numeric(reference_date - latest_date) / 30.44
    
    if (months_elapsed >= prediction_months) {
      message(sprintf("    %s: ✓ Latest case has %.1f months follow-up (>= %d months)",
                     stringr::str_to_title(set_name), 
                     months_elapsed, 
                     prediction_months))
    } else {
      warning(sprintf("    %s: ⚠ Latest case has only %.1f months follow-up (< %d months)",
                     stringr::str_to_title(set_name), 
                     months_elapsed, 
                     prediction_months))
    }
...
#' ランダムなデータ分割（予測期間フィルタ付き）
#' @param df データフレーム
#' @param date_col フィルタ用の日付列（オプション）
#' @param prediction_months 予測期間（月数）
#' @param reference_date 基準日
#' @param train_ratio 訓練データの割合
#' @param val_ratio 検証データの割合
...
random_split <- function(df,
  ...
  message(sprintf("  ✓ Train: n=%d (%.1f%%)", n_train, 100 * train_ratio))
  message(sprintf("  ✓ Val:   n=%d (%.1f%%)", n_val, 100 * val_ratio))
  message(sprintf("  ✓ Test:  n=%d (%.1f%%)", n_test, 100 * test_ratio))
...
stratified_split <- function(df,
  ...
  if (any(is.na(outcome))) {
    message(sprintf("  ⚠ %d rows with missing outcome will be handled separately",
                   sum(is.na(outcome))))
  }
...
#' k-fold クロスバリデーション分割
#' @param df データフレーム（既にフィルタ済みを想定）
#' @param k フォールド数
#' @param stratified 層別化するか
#' @param outcome_col 層別化用のアウトカム列
#' @param seed 乱数シード
#' @return CV分割のリスト
create_cv_folds <- function(df,
  ...
  message(sprintf("\n🔄 Creating %d-fold cross-validation splits...", k))
  ...
  if (stratified && outcome_col %in% names(df)) {
    ...
    for (level in unique(outcome[!is.na(outcome)])) {
      level_idx <- which(outcome == level)
      level_idx <- sample(level_idx)  # シャッフル
  ...
  # フォールドサイズの表示
  message(sprintf("  ✓ Created %d folds", k))
  message(sprintf("  ✓ Fold sizes: min=%d, max=%d, mean=%.1f",
...
save_split <- function(split_result, file_path) {
  ...
  message(sprintf("  ✓ Split saved to: %s", file_path))
}
...
load_split <- function(file_path) {
  ...
  message(sprintf("  ✓ Split loaded from: %s", file_path))
...
#' @param df データフレーム
#' @param config 設定リスト
#' @param method 分割方法（"temporal", "random", "stratified"）
#' @param prediction_months 予測期間（月数）
#' @param reference_date 基準日
#' @param create_cv CVフォールドも作成するか
#' @return 分割結果
split_data_pipeline <- function(df,
  ...
  if (reference_date == "auto" && "op_date" %in% names(df)) {
    ...
    if (optimal_date > Sys.Date()) {
      reference_date <- Sys.Date()
      message("⚠ Latest data too recent, using today as reference")
    } else {
      reference_date <- optimal_date
      message(sprintf("✓ Auto-calculated optimal reference date: %s", reference_date))
    }
  } else if (reference_date == "auto") {
    reference_date <- Sys.Date()
    message("⚠ No op_date column, using today as reference")
  }
  ...
  if (create_cv && nrow(split_result$train) > 0) {
    message("\n📊 Creating CV folds for training data...")
