...
create_time_variables <- function(df) {
  ...
  # ===== Step 1: 必要な基本変数を日本語列名から作成 =====
  
  # 無再発生存期間 → rfs_months
  if (!"rfs_months" %in% names(df) && "無再発生存期間" %in% names(df)) {
    df$rfs_months <- as.numeric(df$無再発生存期間)
    message("  ✓ Created rfs_months from 無再発生存期間")
  }
  
  # 全生存期間m → os_months  
  if (!"os_months" %in% names(df) && "全生存期間m" %in% names(df)) {
    df$os_months <- as.numeric(df$全生存期間m)
    message("  ✓ Created os_months from 全生存期間m")
  }
  
  # 再発有無TRUE/FALSEの二群 → recur_tf
  # バッククォートを使用して列名を正確に指定
  if (!"recur_tf" %in% names(df)) {
    # 可能な列名のパターンを検索
    recur_cols <- grep("再発有無", names(df), value = TRUE)
    if (length(recur_cols) > 0) {
      df$recur_tf <- df[[recur_cols[1]]]  # 最初にマッチした列を使用
      message(sprintf("  ✓ Created recur_tf from %s", recur_cols[1]))
    }
  }
  
  # 生死 → vital_status
  if (!"vital_status" %in% names(df) && "生死" %in% names(df)) {
    df$vital_status <- df$生死
    message("  ✓ Created vital_status from 生死")
  }
  
  # 生1死0 → vital10
  if (!"vital10" %in% names(df) && "生1死0" %in% names(df)) {
    df$vital10 <- df$生1死0
    message("  ✓ Created vital10 from 生1死0")
  }
  
  # ===== Step 2: 派生変数を作成 =====
  
  # 2年打ち切り時間 (time2y) - 最重要変数
  if ("rfs_months" %in% names(df)) {
    df$time2y <- pmin(df$rfs_months, 24, na.rm = FALSE)
    message(sprintf("  ✓ time2y: created (median = %.1f months)",
                   median(df$time2y, na.rm = TRUE)))
  } else if ("os_months" %in% names(df)) {
    df$time2y <- pmin(df$os_months, 24, na.rm = FALSE)
    message("  ✓ time2y: created from os_months (fallback)")
  } else {
  ...
  if (all(c("recur_tf", "rfs_months") %in% names(df))) {
    # recur_tfを論理値に変換
    recur_logical <- df$recur_tf %in% c("TRUE", TRUE, "1", 1, "有")
  ...
  if (all(c("rfs_months", "os_months", "vital_status") %in% names(df))) {
    # 死亡フラグ
    is_dead <- df$vital_status %in% "死" | df$vital10 %in% c("FALSE", FALSE, "0", 0)
...
handle_missing_values <- function(df, config = NULL, method = "median") {
  ...
  if (remaining_missing > 0) {
    message(sprintf("\n  ⚠ Warning: %d variables still have missing values:", 
                   remaining_missing))
...
# ========================================
# 不要な変数を完全に削除して、欠損値を確実に補完
# ========================================

clean_and_impute_completely <- function(data) {
  
  message("\n🧹 Cleaning and imputing data COMPLETELY...")
  
  # Step 1: 不要な変数を削除
  vars_to_remove <- c(
    # BMI_category関連（欠損率45%）
  ...
  if (remaining_na > 0) {
    message(sprintf("\n⚠️ WARNING: Still %d NA values remaining!", remaining_na))
    na_cols <- colSums(is.na(data))
    na_cols <- na_cols[na_cols > 0]
    print(na_cols)
  } else {
    message("\n✅ SUCCESS: All NAs removed!")
  }
