...
evaluate_all_models <- function(models, split_data, config = NULL) {
  ...
  # ===== 重要な修正：prepare_model_dataを使わない =====
  # split_dataはすでに準備済みなので、そのまま使用
  train_data <- split_data$train
  val_data <- split_data$val
  test_data <- split_data$test
  
  # surv_objがない場合のみ作成（安全のため）
  if (!"surv_obj" %in% names(train_data)) {
    if (all(c("time2y", "recur_2y") %in% names(train_data))) {
      train_data$surv_obj <- survival::Surv(
        time = train_data$time2y,
        event = as.numeric(train_data$recur_2y)
      )
    }
  }
  
  if (!"surv_obj" %in% names(val_data)) {
    if (all(c("time2y", "recur_2y") %in% names(val_data))) {
      val_data$surv_obj <- survival::Surv(
        time = val_data$time2y,
        event = as.numeric(val_data$recur_2y)
      )
    }
  }
  
  if (!"surv_obj" %in% names(test_data)) {
    if (all(c("time2y", "recur_2y") %in% names(test_data))) {
      test_data$surv_obj <- survival::Surv(
        time = test_data$time2y,
        event = as.numeric(test_data$recur_2y)
      )
    }
  }
