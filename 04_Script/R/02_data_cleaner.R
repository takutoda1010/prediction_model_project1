knitr::opts_chunk$set(echo = TRUE)

################################################################################
# 02_data_cleaner.R - データクリーニングと前処理
# 
# Description: 
#   データの詳細なクリーニング処理
#   ステージ変数の作成、参照レベル設定、データ型の詳細調整
#
# Author: Takuto Yoshida
# Date: 2024-11-07
################################################################################

# ========================================================================
# 1. ステージ変数の作成
# ========================================================================

#' R/BR群分けステージ変数を作成
#' @param df データフレーム
#' @param config 設定リスト
#' @return ステージ変数が追加されたデータフレーム
create_stage_variables <- function(df, config = NULL) {
  
  message("\n🏷️ Creating stage variables...")
  
  # 手術時群分け（R/BR1/BR2）
  surg_patterns <- c(
    "手術時群分け_R_BR1_BR2",  # 変換後
    "手術時群分け\\s*R\\s*/\\s*BR1\\s*/\\s*BR2"  # 変換前
  )
  # dfの列名からsurg_patternに一致するものを探す。
  surg_cols <- grep(surg_patterns, names(df), value = TRUE, perl = TRUE)
  
  if (length(surg_cols) > 0) {
    df$stage_surg <- factor(
      df[[surg_cols[1]]], 
      levels = c("R", "BR1", "BR2")
    )
    message("  ✓ Created: stage_surg")
  }
  
  # 診断時群分け（R/BR1/BR2）
  diag_patterns <- c(
    "診断時群分け_R_BR1_BR2",  # 変換後
    "診断時群分け_R_BR",  # 変換後の別パターン
    "診断時群分け"
  )
  
  diag_cols <- grep(diag_patterns, names(df), value = TRUE, perl = TRUE)
  
  if (length(diag_cols) > 0) {
    df$stage_diag <- factor(
      df[[diag_cols[1]]], 
      levels = c("R", "BR1", "BR2")
    )
    message("  ✓ Created: stage_diag")
  }
  
  # R/BR二分類（BR1とBR2をBRに統合）
  rb_patterns <- c(
    "^群分け_R_BR",  # 変換後
    "^群分け\\s*R\\s*/\\s*BR"  # 変換前
  )
  
  rb_cols <- grep(rb_patterns, names(df), value = TRUE, perl = TRUE)
  
  if (length(rb_cols) > 0) {
    x <- stringr::str_squish(as.character(df[[rb_cols[1]]]))
    x <- ifelse(x %in% c("BR1", "BR2"), "BR", x)
    df$group_rb <- factor(x, levels = c("R", "BR"))
    message("  ✓ Created: group_rb")
  } else if ("stage_surg" %in% names(df)) {
    # stage_surgから作成
    x <- as.character(df$stage_surg)
    x[x %in% c("BR1", "BR2")] <- "BR"
    df$group_rb <- factor(x, levels = c("R", "BR"))
    message("  ✓ Created: group_rb (from stage_surg)")
  } else if ("stage_diag" %in% names(df)) {
    # stage_diagから作成
    x <- as.character(df$stage_diag)
    x[x %in% c("BR1", "BR2")] <- "BR"
    df$group_rb <- factor(x, levels = c("R", "BR"))
    message("  ✓ Created: group_rb (from stage_diag)")
  }
  
  # 非癌部肝ステージ（旧）
  nc_candidates <- c(
    "StageI+II or III+IV　非癌部肝(旧入力）",
    "StageI+II or III+IV"
  )
  nc_cols <- intersect(nc_candidates, names(df))
  
  if (length(nc_cols) > 0 && !"nc_liver_stage_old" %in% names(df)) {
    df$nc_liver_stage_old <- df[[nc_cols[1]]]
    message("  ✓ Created: nc_liver_stage_old")
  }
  
  return(df)
}

# ========================================================================
# 1.5. 変数名のクリーニング（新規追加）
# ========================================================================

#' 変数名をRで扱いやすい形式に修正
#' @param df データフレーム
#' @return クリーンな変数名のデータフレーム
clean_variable_names <- function(df) {
  
  message("\n🧹 Cleaning variable names...")
  
  # 現在の変数名
  old_names <- names(df)
  new_names <- old_names
  
  # スペースをアンダースコアに置換
  new_names <- gsub(" ", "_", new_names)
  
  # 特殊文字の処理
  new_names <- gsub("±", "_pm", new_names)
  new_names <- gsub("\\+", "_plus", new_names)
  new_names <- gsub("-", "_minus", new_names)
  new_names <- gsub("/", "_", new_names)
  new_names <- gsub("\\(", "_", new_names)
  new_names <- gsub("\\)", "_", new_names)
  new_names <- gsub("\\.", "_", new_names)
  
  # 連続するアンダースコアを1つに
  new_names <- gsub("_+", "_", new_names)
  
  # 末尾のアンダースコアを削除
  new_names <- gsub("_$", "", new_names)
  
  # 変更があった変数名を報告
  changed <- old_names != new_names
  if (any(changed)) {
    message("  Changed variable names:")
    changes <- data.frame(
      old = old_names[changed],
      new = new_names[changed]
    )
    for (i in 1:min(10, nrow(changes))) {
      message(sprintf("    '%s' → '%s'", changes$old[i], changes$new[i]))
    }
    if (nrow(changes) > 10) {
      message(sprintf("    ... and %d more", nrow(changes) - 10))
    }
  }
  
  # 名前を設定
  names(df) <- new_names
  
  return(df)
}

# ========================================================================
# 2. カテゴリ変数の参照レベル設定
# ========================================================================

#' カテゴリ変数の参照レベルを設定
#' @param df データフレーム
#' @param config 設定リスト
#' @param variables 変数定義リスト
#' @return 参照レベルが設定されたデータフレーム
set_reference_levels <- function(df, config = NULL, variables = NULL) {
  
  message("\n⚙️ Setting reference levels for categorical variables...")
  
  # 設定から参照レベルを取得
  ref_levels <- NULL
  if (!is.null(config) && !is.null(config$data_processing$reference_levels)) {
    ref_levels <- config$data_processing$reference_levels
  } else {
    # デフォルトの参照レベル
    ref_levels <- list(
      sex = "F",
      group_rb = "R",
      stage_surg = "R",
      stage_diag = "R",
      vital_status = "生",
      vital10 = "TRUE",
      recur_tf = "FALSE",
      eventfree10 = "FALSE"
    )
  }
  
  # 参照レベルの設定
  for (var_name in names(ref_levels)) {
    if (var_name %in% names(df)) {
      current_levels <- unique(as.character(df[[var_name]]))
      ref_level <- ref_levels[[var_name]]
      
      if (ref_level %in% current_levels) {
        # 参照レベルを先頭に配置
        other_levels <- setdiff(current_levels, ref_level)
        new_levels <- c(ref_level, other_levels)
        
        df[[var_name]] <- factor(
          df[[var_name]], 
          levels = new_levels
        )
        
        message(sprintf("  ✓ %s: reference = '%s'", var_name, ref_level))
      }
    }
  }
  
  # 変数定義からの追加設定
  if (!is.null(variables) && !is.null(variables$column_mappings)) {
    for (mapping in variables$column_mappings) {
      col_name <- mapping$name
      
      if (col_name %in% names(df) && !is.null(mapping$reference)) {
        if (is.factor(df[[col_name]]) || is.character(df[[col_name]])) {
          current_levels <- unique(as.character(df[[col_name]]))
          
          if (as.character(mapping$reference) %in% current_levels) {
            other_levels <- setdiff(current_levels, as.character(mapping$reference))
            new_levels <- c(as.character(mapping$reference), other_levels)
            
            df[[col_name]] <- factor(
              df[[col_name]], 
              levels = new_levels
            )
          }
        }
      }
    }
  }
  
  return(df)
}

# ========================================================================
# 3. 死因分類の作成
# ========================================================================

#' 死因を4分類に整理
#' @param df データフレーム
#' @return 死因分類が追加されたデータフレーム
create_death_cause_categories <- function(df) {
  
  if ("死因" %in% names(df)) {
    message("\n💀 Creating death cause categories...")
    
    y <- stringr::str_squish(as.character(df$死因))
    
    # その他のパターン
    other_pattern <- "(他病死(（老衰）|\\(老衰\\))?|敗血症|死（原因不明）|死\\(原因不明\\))"
    
    df$death_cause4 <- dplyr::case_when(
      is.na(y) | y == "" ~ "aliver",
      stringr::str_detect(y, "癌") ~ "Cancer",
      stringr::str_detect(y, "肝不全") ~ "LiverFailure",
      stringr::str_detect(y, other_pattern) ~ "Other",
      TRUE ~ "Other"
    ) %>%
      factor(levels = c("aliver", "Cancer", "LiverFailure", "Other"))
    
    message("  ✓ Created: death_cause4")
    
    # 集計表示
    death_table <- table(df$death_cause4, useNA = "ifany")
    message("  Distribution:")
    for (i in seq_along(death_table)) {
      message(sprintf("    %s: %d", names(death_table)[i], death_table[i]))
    }
  }
  
  return(df)
}

# ========================================================================
# 4. 順序因子の設定
# ========================================================================

#' 順序因子変数を適切に設定
#' @param df データフレーム
#' @param variables 変数定義リスト
#' @return 順序因子が設定されたデータフレーム
set_ordered_factors <- function(df, variables = NULL) {
  
  message("\n📊 Setting ordered factors...")
  
  # 脈管侵襲グレード（0-4）
  vi_vars <- c("vp_pre", "vv_pre", "vp_path", "vv_path", "va_path", "b_path")
  
  for (var in vi_vars) {
    if (var %in% names(df)) {
      df[[var]] <- ordered(
        df[[var]], 
        levels = c(0, 1, 2, 3, 4)
      )
      message(sprintf("  ✓ %s: ordered (0-4)", var))
    }
  }
  
  # PS (Performance Status)
  if ("ps" %in% names(df)) {
    df$ps <- ordered(df$ps, levels = c(0, 1, 2, 3, 4))
    message("  ✓ ps: ordered (0-4)")
  }
  
  # ASA分類
  if ("asa" %in% names(df)) {
    df$asa <- ordered(df$asa, levels = c(1, 2, 3, 4, 5, 6))
    message("  ✓ asa: ordered (1-6)")
  }
  
  # 合併症グレード
  comp_vars <- c("comp_any_grade", "comp_pleural", "comp_bleeding", "comp_ascites")
  
  for (var in comp_vars) {
    if (var %in% names(df)) {
      df[[var]] <- ordered(df[[var]], levels = c(0, 1, 2, 3))
      message(sprintf("  ✓ %s: ordered (0-3)", var))
    }
  }
  
  # 線維化・炎症スコア
  if ("fibrosis_score" %in% names(df)) {
    df$fibrosis_score <- ordered(df$fibrosis_score, levels = c(0, 1, 2, 3, 4))
    message("  ✓ fibrosis_score: ordered (0-4)")
  }
  
  if ("grading_score" %in% names(df)) {
    df$grading_score <- ordered(df$grading_score, levels = c(0, 1, 2, 3))
    message("  ✓ grading_score: ordered (0-3)")
  }
  
  return(df)
}

# ========================================================================
# 5. 特殊な変数の処理
# ========================================================================

#' 特殊なコーディングの変数を処理
#' @param df データフレーム
#' @return 処理後のデータフレーム
handle_special_variables <- function(df) {
  
  message("\n🔧 Handling special variable codings...")
  
  # fc, fc_inf, sf, sm の処理（-/+ を 0/1に）
  margin_vars <- c("fc", "fc_inf", "sf", "sm")
  
  for (var in margin_vars) {
    if (var %in% names(df)) {
      x <- as.character(df[[var]])
      x <- dplyr::recode(x, "-" = "0", "+" = "1", .default = NA_character_)
      df[[var]] <- factor(x, levels = c("0", "1"))
      message(sprintf("  ✓ %s: converted -/+ to 0/1", var))
    }
  }
  
  # im (intrahepatic metastasis) の処理
  # 0を基準、1/2/3/+は1、S/Xは欠損扱い
  if ("im" %in% names(df)) {
    x <- as.character(df$im)
    
    df$im <- dplyr::case_when(
      is.na(x) ~ NA_character_,
      x %in% c("S", "X") ~ NA_character_,
      x %in% c("0") ~ "0",
      x %in% c("1", "2", "3", "+") ~ "1",
      TRUE ~ NA_character_
    ) %>%
      factor(levels = c("0", "1"))
    
    message("  ✓ im: standardized (0 vs 1+)")
  }
  
  # HBV の処理
  if ("hbv" %in% names(df)) {
    # レベルの確認と統一
    df$hbv <- factor(df$hbv, levels = c("-", "±", "+", "*"))
    message("  ✓ hbv: factor levels set")
  }
  
  # HCV の処理（TRUE/FALSEを0/1に）
  if ("hcv" %in% names(df)) {
    if (is.logical(df$hcv)) {
      df$hcv <- factor(as.integer(df$hcv), levels = c(0, 1))
      message("  ✓ hcv: converted logical to 0/1")
    } else {
      df$hcv <- factor(df$hcv, levels = c(0, 1))
    }
  }
  
  # Child-Pugh分類
  if ("child_pugh" %in% names(df)) {
    df$child_pugh <- factor(df$child_pugh, levels = c("A", "B", "C"))
    message("  ✓ child_pugh: factor levels set (A/B/C)")
  }
  
  # 肝障害度
  if ("liver_damage" %in% names(df)) {
    df$liver_damage <- factor(df$liver_damage, levels = c("A", "B", "C"))
    message("  ✓ liver_damage: factor levels set (A/B/C)")
  }
  
  return(df)
}

# ========================================================================
# 5.5. 日付変数の処理（追加）
# ========================================================================

#' 日付変数を処理してop_dateなどを作成
#' @param df データフレーム
#' @return 処理後のデータフレーム
process_date_variables <- function(df) {
  
  message("\n📅 Processing date variables...")
  
  # to_date_excel_mixed関数が存在するか確認
  if (!exists("to_date_excel_mixed")) {
    # 簡易版の日付変換関数
    to_date_excel_mixed <- function(x) {
      if (inherits(x, "Date")) return(x)
      return(as.Date(x))
    }
  }
  
  # 手術日 → op_date
  if ("手術日" %in% names(df) && !"op_date" %in% names(df)) {
    df$op_date <- to_date_excel_mixed(df$手術日)
    message("  ✓ Created: op_date from 手術日")
  }
  
  # 最終確認日 → last_followup
  if ("最終確認日" %in% names(df) && !"last_followup" %in% names(df)) {
    df$last_followup <- to_date_excel_mixed(df$最終確認日)
    message("  ✓ Created: last_followup from 最終確認日")
  }
  
  # 初診日 → first_visit_dt
  if ("初診日" %in% names(df) && !"first_visit_dt" %in% names(df)) {
    df$first_visit_dt <- to_date_excel_mixed(df$初診日)
    message("  ✓ Created: first_visit_dt from 初診日")
  }
  
  # 入院日 → adm_dt
  if ("入院日" %in% names(df) && !"adm_dt" %in% names(df)) {
    df$adm_dt <- to_date_excel_mixed(df$入院日)
    message("  ✓ Created: adm_dt from 入院日")
  }
  
  # 再発日 → recur_date
  if ("再発日" %in% names(df) && !"recur_date" %in% names(df)) {
    df$recur_date <- to_date_excel_mixed(df$再発日)
    message("  ✓ Created: recur_date from 再発日")
  }
  
  # date of birth → dob
  if ("date of birth" %in% names(df) && !"dob" %in% names(df)) {
    df$dob <- to_date_excel_mixed(df$`date of birth`)
    message("  ✓ Created: dob from date of birth")
  }
  
  return(df)
}

# ========================================================================
# 6. 体積変数の処理
# ========================================================================

#' 体積変数を二値化（0/非0）
#' @param df データフレーム
#' @return 処理後のデータフレーム
process_volume_variables <- function(df) {
  
  message("\n📏 Processing volume variables...")
  
  volume_vars <- c(
    "liver_vol", 
    "tumor_vol", 
    "remnant_vol", 
    "func_resect_rate"
  )
  
  for (var in volume_vars) {
    if (var %in% names(df)) {
      # 元の変数名を保持（_origという名前で）
      df[[paste0(var, "_orig")]] <- df[[var]]
      
      # 二値化（0 vs >0）
      df[[var]] <- factor(
        dplyr::case_when(
          is.na(df[[var]]) ~ NA_real_,
          df[[var]] > 0 ~ 1,
          TRUE ~ 0
        ),
        levels = c(0, 1)
      )
      
      message(sprintf("  ✓ %s: binarized (0 vs >0)", var))
    }
  }
  
  return(df)
}

# ========================================================================
# 7. データ品質チェック
# ========================================================================

#' データ品質の包括的チェック
#' @param df データフレーム
#' @param variables 変数定義リスト
#' @return チェック結果のリスト
check_data_quality <- function(df, variables = NULL) {
  
  message("\n🔍 Performing data quality checks...")
  
  issues <- list()
  
  # 1. 完全重複行のチェック
  dup_rows <- duplicated(df)
  if (any(dup_rows)) {
    issues$duplicate_rows <- which(dup_rows)
    message(sprintf("  ⚠ Found %d duplicate rows", sum(dup_rows)))
  }
  
  # 2. ID重複のチェック
  if ("id" %in% names(df)) {
    dup_ids <- df$id[duplicated(df$id)]
    if (length(dup_ids) > 0) {
      issues$duplicate_ids <- unique(dup_ids)
      message(sprintf("  ⚠ Found %d duplicate IDs", length(unique(dup_ids))))
    }
  }
  
  # 3. 日付の論理チェック
  date_checks <- list(
    list(early = "op_date", late = "recur_date", 
         msg = "Recurrence before surgery"),
    list(early = "op_date", late = "last_followup", 
         msg = "Follow-up before surgery"),
    list(early = "dob", late = "op_date", 
         msg = "Surgery before birth")
  )
  
  for (check in date_checks) {
    if (all(c(check$early, check$late) %in% names(df))) {
      early_dates <- df[[check$early]]
      late_dates <- df[[check$late]]
      
      if (inherits(early_dates, "Date") && inherits(late_dates, "Date")) {
        invalid <- which(!is.na(early_dates) & !is.na(late_dates) & 
                        early_dates > late_dates)
        
        if (length(invalid) > 0) {
          issues[[paste0("date_", check$early, "_", check$late)]] <- invalid
          message(sprintf("  ⚠ %s: %d cases", check$msg, length(invalid)))
        }
      }
    }
  }
  
  # 4. 生存期間の論理チェック
  if (all(c("os_months", "rfs_months") %in% names(df))) {
    # RFS > OS のチェック
    invalid_surv <- which(!is.na(df$os_months) & !is.na(df$rfs_months) & 
                         df$rfs_months > df$os_months)
    
    if (length(invalid_surv) > 0) {
      issues$rfs_exceeds_os <- invalid_surv
      message(sprintf("  ⚠ RFS exceeds OS: %d cases", length(invalid_surv)))
    }
  }
  
  # 5. 範囲外の値チェック
  if (!is.null(variables) && !is.null(variables$column_mappings)) {
    for (mapping in variables$column_mappings) {
      col_name <- mapping$name
      
      if (col_name %in% names(df) && !is.null(mapping$range)) {
        x <- suppressWarnings(as.numeric(df[[col_name]]))
        
        if (any(!is.na(x))) {
          out_of_range <- which(!is.na(x) & 
                               (x < mapping$range[1] | x > mapping$range[2]))
          
          if (length(out_of_range) > 0) {
            issues[[paste0("range_", col_name)]] <- out_of_range
            message(sprintf("  ⚠ %s out of range [%g, %g]: %d cases",
                           col_name, mapping$range[1], mapping$range[2],
                           length(out_of_range)))
          }
        }
      }
    }
  }
  
  # 結果サマリー
  if (length(issues) == 0) {
    message("  ✅ No data quality issues found")
  } else {
    message(sprintf("\n  Total issues found: %d types", length(issues)))
  }
  
  return(issues)
}

# ========================================================================
# 8. 統合クリーニング関数
# ========================================================================

#' データクリーニングパイプライン
#' @param dm DatasetManagerオブジェクト
#' @param config 設定リスト
#' @param variables 変数定義リスト
#' @return 更新されたDatasetManagerオブジェクト
clean_data_pipeline <- function(dm, config = NULL, variables = NULL) {
  
  message("\n========================================")
  message("Starting Data Cleaning Pipeline")
  message("========================================")
  
  # 現在のデータを取得
  df <- dm$get_current()
  current_version <- dm$current_version
  
  # 0. 変数名のクリーニング（最初に実行）← 追加
  df_clean_names <- clean_variable_names(df)
  dm$add_version(df_clean_names, version = current_version + 1)
  
  # 1. ステージ変数の作成（バージョン番号を調整）
  df_stage <- create_stage_variables(df_clean_names, config)
  dm$add_version(df_stage, version = current_version + 2)
  
  # 以下、すべてのバージョン番号を+1する
  # 2. 死因分類の作成
  df_death <- create_death_cause_categories(df_stage)
  dm$add_version(df_death, version = current_version + 3)
  
  # ... 以下同様にバージョン番号を調整
  # 3. 参照レベルの設定 (ds06)
  df_ref <- set_reference_levels(df_death, config, variables)
  dm$add_version(df_ref, version = current_version + 4)
  
  # 4. 順序因子の設定 (ds07)
  df_ordered <- set_ordered_factors(df_ref, variables)
  dm$add_version(df_ordered, version = current_version + 5)
  
  # 5. 特殊変数の処理 (ds08)
  df_special <- handle_special_variables(df_ordered)
  dm$add_version(df_special, version = current_version + 6)
  
  # 5.5. 日付変数の処理 (ds09) ← 追加
  df_dates <- process_date_variables(df_special)
  dm$add_version(df_dates, version = current_version + 7)
  
  # 6. 体積変数の処理 (ds10) ← バージョン番号を調整
  df_volume <- process_volume_variables(df_dates)  # df_special → df_dates に変更
  dm$add_version(df_volume, version = current_version + 8)
  
  # 7. 品質チェック
  quality_issues <- check_data_quality(df_volume, variables)
  
  # 品質問題を属性として保存
  attr(df_volume, "quality_issues") <- quality_issues
  
  # 最終バージョンを更新
  dm$add_version(df_volume, version = current_version + 9)
  
  # サマリー表示
  dm$summary()
  
  message("\n✅ Data cleaning pipeline completed!")
  message("========================================\n")
  
  return(dm)
}

# ========================================================================
# 9. ユーティリティ関数
# ========================================================================

#' クリーニング前後の比較レポート
#' @param dm DatasetManagerオブジェクト
#' @param before_version クリーニング前のバージョン
#' @param after_version クリーニング後のバージョン
#' @return 比較結果
compare_cleaning_results <- function(dm, 
                                    before_version = 3, 
                                    after_version = NULL) {
  
  if (is.null(after_version)) {
    after_version <- dm$current_version
  }
  
  df_before <- dm$get_version(before_version)
  df_after <- dm$get_version(after_version)
  
  message("\n📊 Cleaning Comparison Report")
  message("========================================")
  
  # 列数の変化
  cols_added <- setdiff(names(df_after), names(df_before))
  cols_removed <- setdiff(names(df_before), names(df_after))
  
  message(sprintf("Columns: %d → %d", ncol(df_before), ncol(df_after)))
  
  if (length(cols_added) > 0) {
    message(sprintf("  Added: %s", paste(cols_added, collapse = ", ")))
  }
  
  if (length(cols_removed) > 0) {
    message(sprintf("  Removed: %s", paste(cols_removed, collapse = ", ")))
  }
  
  # 型の変化
  common_cols <- intersect(names(df_before), names(df_after))
  type_changes <- character()
  
  for (col in common_cols) {
    type_before <- class(df_before[[col]])[1]
    type_after <- class(df_after[[col]])[1]
    
    if (type_before != type_after) {
      type_changes <- c(type_changes, 
                       sprintf("%s: %s → %s", col, type_before, type_after))
    }
  }
  
  if (length(type_changes) > 0) {
    message("\nType changes:")
    for (change in head(type_changes, 10)) {
      message("  ", change)
    }
    if (length(type_changes) > 10) {
      message(sprintf("  ... and %d more", length(type_changes) - 10))
    }
  }
  
  message("========================================\n")
  
  invisible(list(
    cols_added = cols_added,
    cols_removed = cols_removed,
    type_changes = type_changes
  ))
}
